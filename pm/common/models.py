...
class MainConfiguration(models.Model):
    ...
    email_host_user = models.EmailField(_(u'EMAIL_HOST_USER'), max_length=255,
                                        help_text=_(u'e.g: youraccount@gmail.com'))
    email_host_password = models.CharField(_(u'EMAIL_HOST_PASSWORD'), max_length=255,
                                           help_text=_(u'Mail account password'))
    email_port = models.PositiveSmallIntegerField(_(u'EMAIL_PORT'), default=587,
                                                  help_text=_(u'Email connection port e.g: 587'))
    ...
    allow_registration = models.BooleanField(_(u'Allow Registration'), default=True,
                                             help_text=_(u'If disabled, Accounts need to be created internally'))
    active_after_registration = models.BooleanField(_(u'Accounts Active After Registration'), default=True,
                                                    help_text=_(u'Enable Accounts after registration automatically'))

    # Authorization
    allow_delete_items = models.BooleanField(_(u'Allow Users to Delete'), default=False,
                                             help_text=_(u'Allow normal users to delete items.'))

    ## Notifications
    email_changes_to_reporter = models.BooleanField(_(u'Notify Reporter'), default=True, help_text=_(
        u'Send Notification to issue reporter for every issue change'))
    email_changes_to_watchers = models.BooleanField(_(u'Notify Watchers'), default=True, help_text=_(
        u'Send Notification to issue watchers for every issue change'))
    email_changes_to_assignee = models.BooleanField(_(u'Notify Assignee'), default=True, help_text=_(
        u'Send Notification to issue assignee for every issue change'))
    email_changes_to_people = models.BooleanField(_(u'Notify People'), default=True, help_text=_(
        u'Send Notification to people assigned to issue for every issue change'))
